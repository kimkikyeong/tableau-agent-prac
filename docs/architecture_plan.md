...
> **구현 현황**: "업무 담당자 가이드라인 → 시스템 프롬프트 바인딩"은 `main_agent.py`의 `TableauAgent._build_system_prompt()`로 구현 완료됐다. `run()` 호출마다 `guidelines` 테이블(제목+본문, 다건 저장)을 조회해 시스템 프롬프트에 "VDS 쿼리 작성 시 우선 참고" 섹션으로 덧붙이므로, 웹 대시보드에서 가이드라인을 저장하면 다음 대화부터 즉시 반영된다(5장 참고). N차 보정 루프·레시피 저장은 Phase 2 미착수 상태.
...
| 입력 컨텍스트 | 필드 원본명·타입·역할·기본 집계 + VDS 통계(결측/고유값/최소/최대/평균) + (선택) `guidelines` 테이블의 전체 업무 가이드라인(`get_guidelines_context_text()`) |
...
## 5. 통합 웹 대시보드 (Chat · 글로서리 큐레이터 · 가이드라인)

읽기 전용 `monitor.html`과 별개로, 실사용자가 직접 조작하는 FastAPI 기반 웹 대시보드를 구축했다.

### 5-1. 구조

```
src/web_app.py            # FastAPI 진입점. lifespan에서 TableauAgent 1회 연결 + /static 마운트 + index.html 서빙
src/routers/
  ├── chat.py              # POST /api/chat — TableauAgent.run() 재사용, 멀티턴 history 지원
  ├── glossary.py          # /api/datasources, /api/glossary, PATCH(승인) /api/glossary/{id}, POST /api/glossary/{id}/regenerate
  └── guideline.py         # /api/guidelines 전체 CRUD (GET 목록/단건, POST, PUT, DELETE)
src/templates/index.html  # Tailwind CDN 기반 SPA. 좌측 사이드바 3탭 + 다크/라이트 토글
src/static/style.css      # color-scheme·nav-active 등 Tailwind CDN이 못 채우는 보강 스타일
```

- FastAPI `lifespan`은 `TableauAgent`(공식 MCP + ta_mcp) 연결을 앱 기동 시 1회만 수행하고, 연결 실패 시에도 예외를 격리해 글로서리·가이드라인 탭은 정상 동작하도록 분리했다.
- 글로서리 큐레이터는 STEP 01의 `db.py` 헬퍼(`get_glossary_rows`, `get_glossary_row`, `upsert_field_business_glossary`)를 그대로 재사용하며, 승인(`is_confirmed=1`)된 필드는 재생성 API에서 409로 거부된다.
- `fetch_step01.py`의 Gemini 호출 로직은 `_call_gemini_glossary()` 공통 헬퍼로 추출되어, 배치 생성(`_generate_datasource_glossary`)과 웹의 단일 필드 재생성(`regenerate_field_glossary`)이 동일 코드를 공유한다.

### 5-2. 분석 가이드라인 — 지표별 다건 저장

업무 담당자가 지표별로 여러 개의 가이드라인(제목+본문)을 저장할 수 있도록 `guidelines` 테이블(id/title/content)을 두고, 저장된 전체를 `get_guidelines_context_text()`로 합쳐 두 곳에 동시 주입한다.

| 소비처 | 반영 시점 |
|---|---|
| `main_agent.py` `TableauAgent._build_system_prompt()` | Agent 대화(`run()`) 호출마다 DB에서 새로 조회 → 시스템 프롬프트에 "VDS 쿼리 작성 시 우선 참고" 섹션으로 추가. 저장 즉시 다음 대화부터 반영 |
| `fetch_step01.py` / `routers/glossary.py` | STEP 01 배치 생성 및 웹의 단일 필드 재생성 시 Gemini 프롬프트에 참고 컨텍스트로 포함 |
...
| 웹 대시보드 확장 | `web_app.py`/`routers/*`/`templates/index.html`로 Agent 대화·글로서리 큐레이터·가이드라인 편집 통합 UI 구축 완료 (5장 참고). 공식 MCP 서브프로세스 인증 env 누락 버그도 함께 수정하여 Chat 탭 정상 동작 확인 |
| 가이드라인 바인딩 | `guidelines` 테이블(제목별 다건) + `TableauAgent._build_system_prompt()`로 "가이드라인 → 시스템 프롬프트 바인딩" 선반영 완료 (원래 Phase 2 항목이었으나 웹 대시보드 작업과 함께 앞당겨 구현) |

### Phase 2 — 정합성 보정 루프 및 레시피 관리

**목표**: VDS 결과와 Tableau View 실제 수치를 자동 대조하여 오차 발생 시 추출 인자를 N차 재보정하고, 검증된 조합을 레시피로 저장·재사용한다. 메타데이터 자동 프로파일링 레이어를 구현한다. (업무 담당자 가이드라인 바인딩은 Phase 1에서 선반영 완료)

| 항목 | 내용 |
|---|---|
| 완료 기준 | 오차 임계값(±1%) 초과 시 자동 재보정 3회 이내 수렴, 레시피 파일 저장 및 재호출 성공 |
| 핵심 구현 | `RecipeStore`(레시피 저장소), `ValidationLoop`(N차 보정), `MetadataProfiler`(자동 프로파일링) |
